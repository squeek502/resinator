...
test "basic rcdata" {
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "file.bin", .data = "hello world" });
...
test "filenames as numeric expressions" {
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "-1", .data = "hello world" });
    try testCompileWithOutput(
        "1 RCDATA -1",
        "\x00\x00\x00\x00 \x00\x00\x00\xff\xff\x00\x00\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x0b\x00\x00\x00 \x00\x00\x00\xff\xff\n\x00\xff\xff\x01\x00\x00\x00\x00\x000\x00\t\x04\x00\x00\x00\x00\x00\x00\x00\x00hello world\x00",
        tmp_dir.dir,
    );
    try tmp_dir.dir.deleteFile(std.testing.io, "-1");
...
test "filename that refers to a directory" {
    ...
    try tmp_dir.dir.createDirPath(std.testing.io, "dir");
    try testCompileErrorDetailsWithDir(
        &.{
...
const test_riff_data = "RIFF\x2C\x00\x00\x00ACONanih\x24\x00\x00\x00" ++ @as([4 * 8]u8, @splat(0)) ++ "\x01\x00\x00\x00";
// A 1x1 png
...
test "uncommon icons/cursors" {
...

    {
        ...
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "riff_in_dir.cur", .data = resdir_riff ++ test_riff_data });
        // switch type to ICON
        resdir_riff[2] = 1;
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "riff_in_dir.ico", .data = resdir_riff ++ test_riff_data });
...

    {
        ...
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "png_in_dir.ico", .data = resdir_png ++ test_png_data });
        resdir_png[2] = 2; // cursor
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "png_in_dir.cur", .data = resdir_png ++ test_png_data });
...

    {
        ...
        resdir_dib[0x16] = 12;
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "old_version.ico", .data = &resdir_dib });
        resdir_dib[2] = 2; // cursor
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "old_version.cur", .data = &resdir_dib });
        ...
        resdir_dib[2] = 1; // icon
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "v5.ico", .data = &resdir_dib });
        resdir_dib[2] = 2; // cursor
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "v5.cur", .data = &resdir_dib });
        ...
        resdir_dib[2] = 1; // icon
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "unknown.ico", .data = &resdir_dib });
        resdir_dib[2] = 2; // cursor
        try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "unknown.cur", .data = &resdir_dib });
...
test "basic bitmap" {
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "test.bmp", .data = "BM<\x00\x00\x00\x00\x00\x00\x006\x00\x00\x00(\x00\x00\x00\x01\x00\x00\x00\x01\x00\x00\x00\x01\x00\x10\x00\x00\x00\x00\x00\x06\x00\x00\x00\x12\x0b\x00\x00\x12\x0b\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\x7f\x00\x00\x00\x00" });
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "test_extra_palette_bytes.bmp", .data = "BM<\x00\x00\x00\x00\x00\x00\x00:\x00\x00\x00(\x00\x00\x00\x01\x00\x00\x00\x01\x00\x00\x00\x01\x00\x10\x00\x00\x00\x00\x00\x06\x00\x00\x00\x12\x0b\x00\x00\x12\x0b\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\x7f\x00\x00\x00\x00" });
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "test_missing_palette_bytes.bmp", .data = "BM<\x00\x00\x00\x00\x00\x00\x006\x00\x00\x00(\x00\x00\x00\x01\x00\x00\x00\x01\x00\x00\x00\x01\x00\x10\x00\x00\x00\x00\x00\x06\x00\x00\x00\x12\x0b\x00\x00\x12\x0b\x00\x00\x04\x00\x00\x00\x00\x00\x00\x00\xff\x7f\x00\x00\x00\x00" });
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "test_win2.0_missing_palette_bytes.bmp", .data = "BMJX\x02\x00\x00\x00\x00\x00G\x00\x00\x00\x0c\x00\x00\x00\x80\x02\xe0\x01\x01\x00\x04\x00\x00\x00\x00\x80\x00\x00\x00\x80\x00\x80\x80\x00\x00\x00\x80\x80\x00\x80\x00\x80\x80\x80\x80\x80\xcc\xcc\xcc\xff\x00\x00\x00\xff\x00\xff\xff\x00\x00\x00\xff\xff\x00\xff\x00\xff\xff" });
    try testCompileErrorDetailsWithDir(
        &.{
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "test_too_many_colors_for_bit_depth.bmp", .data = "BM<\x00\x00\x00\x00\x00\x00\x006\x00\x00\x00(\x00\x00\x00\x01\x00\x00\x00\x01\x00\x00\x00\x01\x00\x10\x00\x00\x00\x00\x00\x06\x00\x00\x00\x12\x0b\x00\x00\x12\x0b\x00\x00\xff\xff\xff\xff\x00\x00\x00\x00\xff\x7f\x00\x00\x00\x00" });
    try testCompileErrorDetailsWithDir(
...
test "basic html" {
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "test.html", .data = "hello" });
...
test "basic stringtable" {
    ...
    try testCompileWithOutputAndOptions(
        ...
        .{ .cwd = .cwd(), .default_code_page = .utf8 },
    );
    // Even if the output code page is 1252
    try testCompileWithOutputAndOptions(
        "#pragma code_page(65001)\nSTRINGTABLE { 1 \"\x80\" }",
        "\x00\x00\x00\x00 \x00\x00\x00\xff\xff\x00\x00\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\"\x00\x00\x00 \x00\x00\x00\xff\xff\x06\x00\xff\xff\x01\x00\x00\x00\x00\x000\x10\t\x04\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\xfd\xff\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00",
        .{ .cwd = .cwd(), .default_code_page = .windows1252 },
    );
...
test "font resource" {
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "empty.fnt", .data = "" });
...
test "aniicon, anicursor" {
    ...
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "test.ani", .data = test_riff_data });
...
test "search paths" {
    ...
    try tmp_dir.dir.createDirPath(std.testing.io, "inc");
    try tmp_dir.dir.writeFile(std.testing.io, .{ .sub_path = "inc/some_crazy_file.bin", .data = "foo" });
...
test "escaped tab characters within strings" {
    ...
    try testCompileWithOutput(source, expected, .cwd());
}
...
test "DLGINCLUDE strings" {
    ...
    try testCompileWithOutput(source, windows1252_expected_output, .cwd());
...
fn testCompileWithOutput(source: []const u8, expected_output: []const u8, cwd: std.Io.Dir) !void {
...
fn testCompileWithOutputAndOptions(source: []const u8, expected_output: []const u8, options: TestCompileOptions) !void {
    ...
    resinator.compile.compile(std.testing.allocator, io, source, &buffer.writer, .{
        .cwd = options.cwd,
        ...
        error.ParseError, error.CompileError => {
            try diagnostics.renderToStderr(io, options.cwd, source, null);
            return err;
        },
        else => return err,
    };
...
fn testCompileErrorDetails(expected_details: []const ExpectedErrorDetails, source: []const u8, maybe_expected_output: ?[]const u8) !void {
    return testCompileErrorDetailsWithDir(expected_details, source, maybe_expected_output, .cwd());
}

fn testCompileErrorDetailsWithDir(expected_details: []const ExpectedErrorDetails, source: []const u8, maybe_expected_output: ?[]const u8, cwd: std.Io.Dir) !void {
